import{a as r}from"./presentation-Dc4PgcXr.js";const m="/api";async function l(c,i={}){const n={headers:{"Content-Type":"application/json",Authorization:`Bearer ${localStorage.getItem("mizaniat_admin_token")}`},...i};n.body&&typeof n.body=="object"&&(n.body=JSON.stringify(n.body));const a=await fetch(`${m}${c}`,n);if(!a.ok){const t=await a.json().catch(()=>({error:a.statusText}));throw new Error(t.error||`HTTP ${a.status}`)}return a.json()}async function u(c){c.innerHTML=`
