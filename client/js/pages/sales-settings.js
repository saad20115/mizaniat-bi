...
export async function renderSalesSettings(container) {
  container.innerHTML = `
    <div class="page-header">
      <div>
        <h1 class="page-title">🛍️ إعدادات نظام المبيعات</h1>
        <p class="page-subtitle">إدارة الاتصال بنظام المبيعات والشركات واستيراد البيانات</p>
      </div>
    </div>
    ...
    <div class="settings-section card" style="margin-top:var(--space-lg);border:1px solid rgba(245,158,11,0.3);">
      ...
      <p style="color:var(--text-muted);font-size:0.8rem;margin-bottom:var(--space-lg);">
        يتم استيراد الفواتير على مراحل (دفعات). كل دفعة = 500 فاتورة. يمكنك متابعة التقدم أثناء الاستيراد.
      </p>
  ...
  btnTest.addEventListener('click', async () => {
    let url = container.querySelector('#conn-url').value.trim();
    ...
    if (!url.startsWith('http://') && !url.startsWith('https://')) {
      url = 'https://' + url;
      container.querySelector('#conn-url').value = url;
    }
    ...
    try {
      const result = await api.sales.testConnectionDirect({ url, username, password, company_id: 1 });
  ...
  btnSave.addEventListener('click', async () => {
    ...
    try {
      ...
      await api.sales.createInstance({
        name: 'المبيعات - ' + hostname,
        url: url,
        username: username,
        api_key: password,
      });
  ...
  container.querySelector('#btn-save-schedule').addEventListener('click', async () => {
    ...
    try {
      const result = await api.sales.updateSchedule({
  ...
  container.querySelector('#btn-clear-notifs').addEventListener('click', async () => {
    try {
      await api.sales.clearNotifications();
...
async function loadSettingsData(container) {
  try {
    const [instances, companies, rules, syncStatus, schedule, notifications] = await Promise.all([
      api.sales.getInstances().catch(() => []),
      api.sales.getCompanies().catch(() => []),
      api.sales.getEliminationRules().catch(() => []),
      api.sales.getSyncStatus().catch(() => []),
      api.sales.getSchedule().catch(() => ({ enabled: false, intervalHours: 2, running: false })),
      api.sales.getNotifications().catch(() => [])
    ]);
...
function renderInstances(el, instances, mainContainer) {
  ...
  el.querySelectorAll('.btn-delete-instance').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        await api.sales.deleteInstance(id);
...
function renderCompanies(el, companies, instances, mainContainer) {
  ...
  el.innerHTML = `
    <div class="data-table-wrapper">
      <table class="data-table">
        <thead><tr><th>اسم الشركة</th><th>الخادم المربوط</th><th>Sales ID</th><th>عدد الفواتير</th><th>آخر فاتورة</th><th>الحالة</th><th>إجراء</th></tr></thead>
        <tbody>
          ${companies.map(c => {
            const inst = instMap[c.sales_instance_id];
            const instOptions = instances.map(i =>
              `<option value="${i.id}" ${i.id === c.sales_instance_id ? 'selected' : ''}>${i.name}</option>`
            ).join('');
            return `
            <tr>
              <td>
              ...
              <td style="font-family:var(--font-en);">${c.sales_company_id}</td>
              ...
              <td>
                <button class="btn btn-sm btn-delete-company" data-id="${c.id}" style="background:rgba(239,68,68,0.1);color:var(--accent-red);border:1px solid rgba(239,68,68,0.2);font-size:0.75rem;padding:3px 8px;">🗑️</button>
              </td>
            </tr>
          `}).join('')}
        </tbody>
  ...
  el.querySelectorAll('.company-instance-select').forEach(sel => {
    sel.addEventListener('change', async () => {
      ...
      try {
        await api.sales.updateCompany(companyId, { sales_instance_id: newInstanceId });
  ...
  el.querySelectorAll('.btn-delete-company').forEach(btn => {
    btn.addEventListener('click', async () => {
      const companyId = btn.dataset.id;
      if (!confirm('هل أنت متأكد من حذف هذه الشركة وحذف جميع بيانات الفواتير المرتبطة بها نهائياً؟')) return;
      try {
        await api.sales.deleteCompany(companyId);
...
function renderImportPanel(el, companies) {
    ...
    <div class="card" style="margin-bottom:var(--space-md);border:1px solid rgba(255,255,255,0.05);padding:var(--space-md);" id="import-card-${c.id}">
      <div style="display:flex;justify-content:space-between;align-items:center;">
        <div>
          ...
          <span style="font-family:var(--font-en);color:var(--text-muted);font-size:0.8em;margin-right:8px;">(Company ID: ${c.sales_company_id})</span>
          <span style="font-family:var(--font-en);color:var(--text-muted);font-size:0.8em;">— ${formatNumber(c.item_count || 0)} فاتورة</span>
        </div>
  ...
  el.querySelectorAll('.btn-import').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        await api.sales.syncCompany(companyId);
...
function startProgressPolling(companyId) {
  ...
  progressTimers[companyId] = setInterval(async () => {
    try {
      const progress = await api.sales.getSyncProgress(companyId);
      ...
      if (progress.status === 'completed' || progress.status === 'failed') {
        ...
        if (progress.status === 'completed') {
          showToast(`تم استيراد ${formatNumber(progress.inserted)} فاتورة بنجاح ✅`, 'success');
          // Refresh companies list
          const companies = await api.sales.getCompanies();
          store.set('sales_companies', companies);
        } else {
...
async function startAllProgressPolling(container) {
...
function renderSyncLog(el, logs) {
  ...
  el.innerHTML = `
    <div class="data-table-wrapper">
      <table class="data-table">
        <thead><tr><th>الشركة</th><th>الحالة</th><th>الفواتير المستوردة</th><th>البداية</th><th>النهاية</th><th>الخطأ</th></tr></thead>
        <tbody>
...
function showAddCompanyModal(mainContainer) {
  showModal('إضافة شركة جديدة', `
    ...
    <div class="form-group">
      <label class="form-label">معرف الشركة في المبيعات (Company ID)</label>
      <input type="number" class="form-input" id="comp-sales-id" placeholder="1" dir="ltr" style="font-family:var(--font-en);" />
      <small style="color:var(--text-muted);font-size:0.72rem;">هذا هو الرقم في ?company_id=X في رابط الـ API</small>
    </div>
    <div class="form-group">
      <label class="form-label">اتصال المبيعات</label>
  ...
  api.sales.getInstances().then(instances => {
    const sel = document.getElementById('comp-instance');
    if (instances.length === 0) {
      sel.innerHTML = '<option disabled>⚠️ أضف اتصال المبيعات أولاً</option>';
    } else {
  ...
  document.getElementById('btn-save-company').addEventListener('click', async () => {
    ...
    const salesId = parseInt(document.getElementById('comp-sales-id').value);
    ...
    try {
      await api.sales.createCompany({
        name: name,
        sales_company_id: salesId,
        sales_instance_id: instId,
...
function showAddRuleModal(mainContainer) {
  const companies = store.get('sales_companies');
  ...
  document.getElementById('btn-save-rule').addEventListener('click', async () => {
    try {
      await api.sales.createEliminationRule({
